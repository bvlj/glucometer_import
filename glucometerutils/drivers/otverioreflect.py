...
"""
Driver for LifeScan OneTouch Verio Reflect.
...
from glucometerutils.support import lifescan, lifescan_binary_protocol, udev
...
class Device(driver.GlucometerDevice):
    def __init__(self, device: Optional[str]) -> None:
        if not device:
            # Try to automatically detect the path using udev
            device = udev.retrieve_usb_disk_device_node(0x2766, 0x000C)
        if not device:
            raise exceptions.CommandLineError("Device detection failed.")
